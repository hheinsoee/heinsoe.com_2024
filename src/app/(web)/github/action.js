...
export async function getGitProjects(params) {
  ...
  const response = await fetch(
    `https://api.github.com/search/repositories?q=user:${me.githubUsername}+fork:false&sort=stars&per_page=10&type=Repositories&page=${page}`,
...
export async function getGitProfile() {
  const url = `https://api.github.com/users/${me.githubUsername}`;
